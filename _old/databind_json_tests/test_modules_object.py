...
def test_unknown_keys():
  ...
  assert str(excinfo.value) == """unknown keys found while deserializing ObjectType(test_modules_object.test_unknown_keys.<locals>.Person): {'age'}
  Conversion trace:
    - $ (ObjectType(test_modules_object.test_unknown_keys.<locals>.Person)) [None]"""
...
def test_map_remainders():
  ...
  assert str(excinfo.value) == """key 'port' of remainder field 'rest' cannot be exploded into resulting JSON object because of a conflict.
  Conversion trace:
    - $ (ObjectType(test_modules_object.test_map_remainders.<locals>.Config)) [None]"""
